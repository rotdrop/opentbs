...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
	 ...
	 */
	function TbsPicExternalPath(&$Value, &$PrmLst, $Loc, &$DataType) {
	 ...
	 * @param string  $Txt
	 * @param object  $Loc
	 * @param array   $ImagesLoc An array
	 *
	 * [
	...
	function TbsPicAdd(&$Value, &$PrmLst, &$Txt, &$Loc, $ImagesLoc, $Prm) {
		...
		$ExternalPath = $this->TbsPicExternalPath($Value, $PrmLst, $Loc, $DataType);
		...
		if ($this->FileGetIdxAdd($InternalPath)===false) {
			$this->FileAdd($InternalPath, $ExternalPath, $DataType, true);
		}
